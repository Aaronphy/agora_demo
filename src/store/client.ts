...
const DEFAULT = {
    appId: getQueryString('appId') || '09159282a952477a8b5edac5c8b463a2',
    channel: getQueryString('channel') || 'test',
    token:
        getQueryString('token') ||
        '00609159282a952477a8b5edac5c8b463a2IABkWQrtPAQUDMLoDcrQ7GTzZ1i2vYs8A8UyZz+GfyOv6Ax+f9gAAAAAEADGU1aHxuKzXwEAAQDF4rNf',
};
...
export default class ClietStore {
    ...
    @observable.ref
    cameraResolution: string[] = ['default', '480p', '720p', '1080p'];
    ...
    @action
    async join(values: Partial<IParams>) {
        let err: any, res: any;
    ...
    @action
    leave() {
        if (!this.client) return this.notice('Please Join First', 'error');
        if (!this.joined) return this.notice('You are not in channel', 'error');
        this.client.leave(
            () => {
                ...
                for (let i = 0; i < this.remoteStreams.length; i++) {
                    var stream = <RTC.Stream>this.remoteStreams.shift();
                    var id = stream.getId();
                    if (stream.isPlaying()) {
                        stream.stop();
                    }
                ...
                this.joined = false;
                this.notice('leave success', 'success');
            },
            () => {
                this.notice('leave success', 'error');
            }
        );
    }
    ...
    @action
    unpublish() {
        if (!this.client) return this.notice('Please Join Room First');
        if (!this.published) return this.notice('Your didn`t publish');

        let oldState = this.published;
        this.client.unpublish(this.localStream as RTC.Stream, (err) => {
            this.published = oldState;
            console.log('unpublish failed');
            this.notice('unpublish failed', 'error');
            console.error(err);
        });
        this.notice('unpublish');
        this.published = false;
    }
    ...
    notice(message: string, type: string = 'info') {
        notification[type]({ message, duration: 2 });
    }
}
